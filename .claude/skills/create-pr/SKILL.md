...
API 추가/변경이 있으면, 미리보기를 만들기 전에 로컬 서버로 대표 요청의 응답 예시를 만들어 "확인 자료"에 넣는다.
- 이미 떠 있는 서버(8080)가 있으면 그 서버를 쓰고, 없으면 띄운 뒤 끝나면 종료한다.
- 데이터를 바꾸는 요청(POST/PUT/PATCH/DELETE)은 실행하기 전에 사용자에게 확인받는다.
...
공통
- diff를 보면 알 수 있는 내용은 쓰지 않는다. 메서드·필드 이름을 나열하거나 시그니처를 옮겨 적지 않는다.
- 화살표(→)나 명사 나열 대신 완결된 문장으로 쓴다.
- 리스트 중첩은 1단계까지.
- 코드 표기(``)는 리뷰어가 검색하거나 찾아가야 할 이름에만 쓴다.

요약
- 1문장. 사용자(프론트, 다른 개발자) 입장에서 달라지는 점을 쓴다.
  - 예: "전역 예외 처리를 core에 추가했습니다"(X) → "API 실패 응답이 `{status, code, message}` 형식으로 통일됩니다"(O)

작업 내용
- 한 줄에 한 가지 변화.
- 각 줄에 "무엇을 만들었나 + 그래서 무엇이 달라지나"를 함께 쓴다. 만든 것은 핵심 이름 하나(클래스 이름 수준)로만 짚는다.
  - 예: "`BaseException`을 추가해서, 이를 상속한 예외를 던지기만 하면 에러 응답이 만들어집니다"
