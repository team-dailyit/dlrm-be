---
name: review-task
description: 리뷰 대상(기준점·diff)을 확인하고 검증(format, build+test)을 실행한 뒤, Standards(컨벤션 준수)와 Spec(티켓 요구사항 충족) 두 축으로 나누어 병렬 서브에이전트로 리뷰하고 결과를 나란히 보고한다. 개발이 끝났거나 리뷰 반영 후 재검증이 필요할 때 사용한다.
allowed-tools: Bash(git fetch:*), Bash(git diff:*), Bash(git log:*), Bash(git rev-parse:*), Bash(git merge-base:*), Bash(git ls-files:*), Bash(./gradlew spotlessCheck:*), Bash(./gradlew build:*), Bash(git branch:*), Bash(docker info:*), mcp__atlassian__getAccessibleAtlassianResources, mcp__atlassian__getJiraIssue, Agent
---

# review-task

리뷰 대상 확인, 검증, 코드 리뷰를 순서대로 진행한다. 리뷰 대상이 없거나 검증이 실패하면 리뷰는 진행하지 않는다.
...
- 기준점이 유효하지 않거나 diff와 새 파일이 모두 비어있으면 여기서 멈추고 사용자에게 알린다. (빌드·테스트를 돌리거나 서브에이전트를 띄운 뒤 실패하는 것보다 미리 걸러낸다)

## 2단계: 검증

아래 명령을 순서대로 실행한다. 하나라도 실패하면 즉시 중단한다. (CI가 구성되면 `.github/workflows/ci.yaml`과 동일한 명령·순서를 유지한다)

1. `./gradlew spotlessCheck` — 포맷 검사
2. `./gradlew build` — 컴파일 + 테스트

**실패 시**

- 리뷰를 진행하지 않는다.
- `build`가 실패하면 먼저 Docker가 실행 중인지 확인한다(`docker info`). 테스트가 Testcontainers를 쓰므로, Docker가 꺼져 있으면 코드 문제가 아닌데도 테스트가 실패한다. 이 경우 Docker 실행을 안내하고 코드 문제로 보고하지 않는다.
...
- 문서가 커버하지 못하는 부분은 아래 **스멜 baseline**을 판단 기준(heuristic)으로 사용한다.
  ...
  - Spotless가 이미 정리하는 항목(포맷, 미사용 import 등)은 스멜로 다시 지적하지 않는다.
...
- 지시: "파일/hunk 단위로 (a) 문서화된 컨벤션 위반 사항 — 어떤 규칙을 어겼는지 인용, (b) baseline 스멜 — 이름과 해당 hunk 인용, 을 보고할 것. 확정 위반(문서화된 컨벤션)과 판단성 지적(스멜)을 구분할 것. 스멜은 항상 판단성 지적이며, 문서화된 규칙이 스멜보다 우선한다. Spotless가 이미 정리하는 건 생략. 반드시 고쳐야 하는 것과 제안 수준을 구분해서 정리."
...
- **Repeated Switches** — 같은 타입에 대한 switch/if-분기가 여러 곳에서 반복됨 → 다형성(또는 enum에 동작을 두는 방식)으로 대체.
